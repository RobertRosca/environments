...
for environment in environments:
    ...
    with mkdocs_gen_files.open(PAGE_PREFIX + page, "w") as f:
        text = f"# `{name}`\n"
    ...
    mkdocs_gen_files.set_edit_path(PAGE_PREFIX + page, "gen_pages.py")

    ENVIRONMENTS_DICT[name] = page

mkdocs_gen_files.set_edit_path(PAGE_PREFIX + "environments.md", "gen_pages.py")

index_file = Path(__file__).parent / "environments.md"
index = index_file.read_text()

with mkdocs_gen_files.open(f"{PAGE_PREFIX}environments.md", "w") as f:
    text = "".join(
        f"- [{name}]({filename})\n" for name, filename in ENVIRONMENTS_DICT.items()
    )
    f.write(index.replace("{{ ENVIRONMENT_LIST }}", text))

with mkdocs_gen_files.open(f"{PAGE_PREFIX}environments/SUMMARY.md", "w") as nav_file:
